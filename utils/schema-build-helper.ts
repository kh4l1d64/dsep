...
export const buildContext = (input: any = {}) => {
  ...
  const context: ITrainingNetworkContext = {
    ...
    action: input.action ?? "",
    version: `${process.env.CORE_VERSION || (input?.core_version ?? "1.1.0")}`,
    ...
    timestamp: input.timestamp ?? currentMoment.toISOString(),
    ttl: input?.ttl ?? "PT10M"
  };
  return context;
};
...
export const buildSearchRequest = (input: any = {}) => {
  ...
  console.log("At the end of buildSearchRequest");
